...
(defun color-at (scn x y)
  (let* ((vr (lm:make-vector 3 :initial-elements (list (- x (point-x (scene-eye scn)))
						       (- y (point-y (scene-eye scn)))
						       (- 0 (point-z (scene-eye scn))))))
	 (r (make-ray :origin (scene-eye scn) :direction (lm:normalise vr))))
    (round (* (sendray scn r) 255))))

(defun sendray (scn r)
  (multiple-value-bind (s int) (first-hit scn r)
    (if s
	(* (lambert s int (ray-direction r)) (surface-color s))
	0)))
...
(defun init-scene ()
  (let ((surfaces nil))
    (push (create-sphere 0 -300 -1200 200 .8) surfaces)
    (push (create-sphere -80 -150 -1200 200 .7) surfaces)
    (push (create-sphere 70 -100 -1200 200 .9) surfaces)
    (do ((x -2 (1+ x)))
	((> x 2))
      (do ((z 2 (1+ z)))
	  ((> z 7))
	(push (create-sphere (* x 200) 300 (* z -400) 50 .75) surfaces)))
    surfaces))

(defun render (pathname)
  (let* ((scn (make-scene :eye (make-point :x 0 :y 0 :z 200) :surfaces (init-scene)))
	 (width 640) 
	 (height 480)
	 (res 3)
